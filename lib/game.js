...
function setUpConnection() {
  ...
  if (!isHost) {
    rtcConnection.ondatachannel = onDataChannel;
  } else {
    ...
    rtcConnection.createOffer().then(gotDescription);
  }
}
...
function onChannelOpen() {
  ...
  if (ffStream) {
    ffStream.getTracks().forEach((track) => {
      track.stop();
      ffStream.removeTrack(track);
    });
  }
}
...
//
// { type: "ready", shapeLocations: [[id1, id2...], [id3, id4...]], playerName: playerName, currentTime: currentTime }
//
...
// gameState includes the gameConfig properties from setup.js, plus the following properties: playerHits,
// playerMisses, opponentHits, opponentMisses, playerShapes, opponentShapes, gameStartTime,
// nextTimerAlert, playerName, opponentName, timeOffset.
...
function sendReadyMessage(shapeLocations, playerName, currentTime) {
  sendMessage("ready", {
...
function opponentIsReady(shapePositions, opponentName, currentTime) {
  if (isHost) {
    gameState.timerOffset = new Date() - currentTime;
  }
...
function startGame() {
  ...
  sendGameStartMessage(startTime - gameState.timerOffset);
...
var jPlayerTiles, jOpponentTiles;
...
function setUpGame() {
  ...
  $("#searchBox")
    .on("input", (e) => {
      ...
      foundPokes.forEach((num) => {
        if (jOpponentTiles[num]) {
          jOpponentTiles[num].addClass("queried");
          jPlayerTiles[num].addClass("queried");
        }
      });
    })
    .on("blur", (e) => {
  ...
  $("#readyButton").click((e) => {
    ...
    sendReadyMessage(gameState.playerShapes, playerName, new Date() - 0);
...
function setUpBoards() {
  ...
  jOpponentTiles = createBoard(
    pokeSquareOpponent,
    jOpponentBoard,
    pokeSet,
    numColumns
  );
  jPlayerTiles = createBoard(
    pokeSquarePlayer,
    jPlayerBoard,
    pokeSet,
    numColumns
...
function queryPokes(query) {
  ...
  if (!isNaN(queryAsInt)) {
    return pokeSet.filter((num) => num !== queryAsInt);
  } else {
...
function startTimer(startTime) {
  gameState.gameStartTime = gameStartTime = startTime;
